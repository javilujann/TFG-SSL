...
La siguiente gran familia que vamos a presentar es la de los métodos de preprocesado no-supervisado de datos. La idea de esta familia es usar los datos no etiquetados en un paso previo al entrenamiento supervisado del modelo, con el objetivo de mejorar su rendimiento o reducir su complejidad \cite{van2020survey}.
...
Cada método de esta familia se distingue del resto por cómo implementa este paso previo, aun así podemos identificar tres grandes categorías o subfamilias: Las basadas en la extracción de características, las basadas en el \textit{clustering} y las basadas en el pre-entrenamiento de modelos.
...
Uno de los métodos más conocidos de esta categoría es el \gls{PCA}, que busca encontrar una nueva representación de los datos en un espacio de menor dimensionalidad, donde las nuevas dimensiones son combinaciones lineales de las dimensiones originales que capturan la mayor parte de la varianza de los datos \cite{abdi2010principal}. El porqué de usar la varianza como criterio viene del siguiente teorema \cite{burges2010geometric}:
...
Quedando establecido por qué se toma este criterio, el \gls{PCA} se implementa calculando la matriz de covarianza de los datos $C = \frac{1}{n} \sum_{i=1}^n \mathbf{x}_i \mathbf{x}_i^T$. Pues, maximizar la varianza es equivalente a maximizar la expresión $\sum_{j=1}^{d'} \mathbf{v}_j^T C \mathbf{v}_j$ bajo la restricción de que los vectores $\{\mathbf{v}_j\}$ sean ortonormales. Este problema de optimización se resuelve usando métodos analíticos y su solución son los autovectores de la matriz de covarianza $C$, de los cuales se eligen los $d'$ de mayor valor propio \cite{burges2010geometric}.

El \gls{PCA} es solo un ejemplo de método de extracción de características, pero existen muchos otros, como, por ejemplo, los métodos basados en Autoencoders, que buscan aprender una representación comprimida de los datos mediante redes neuronales \cite{LI2023110176}. En definitiva, La idea que todos tienen en común es hacer uso de los datos no etiquetados para transformar la representación de los datos de entrada, con el objetivo de mejorar el rendimiento del modelo supervisado o reducir su complejidad. 

\section{Cluster-then-label}

Los métodos de \textit{cluster-then-label} cambian la idea anterior de realizar una transformación sobre los datos, por la idea de realizar una partición de los datos en clústeres, y utilizar esta división del espacio para facilitar el entrenamiento del modelo supervisado. Es decir, el paso previo en esta categoría consiste en aplicar un algoritmo de \textit{clustering} sobre todo el conjunto de datos, independientemente de si están etiquetados o no \cite{van2020survey}. 
...
donde $\mathbb{I}(\cdot)$ es la función indicadora. Esta formulación permite que la estructura subyacente revelada por los datos no etiquetados reduzca drásticamente la complejidad funcional del aprendizaje supervisado posterior.

Al igual que antes, existen diferentes enfoques a la hora de aplicar esta idea que varían en función del algoritmo de \textit{clustering} elegido y de la forma en la que se utiliza la partición obtenida para mejorar el rendimiento del modelo supervisado.

Un posible enfoque es el propuesto por \citet{pmlr-v5-goldberg09a}, que se basa en la idea de que los datos pueden estar distribuidos en diferentes subespacios, y que cada uno de estos subespacios puede ser modelado por un algoritmo de \textit{clustering} diferente. En este enfoque, se aplica un algoritmo de \textit{clustering} no supervisado para identificar estos subespacios, y luego se entrena un modelo supervisado para cada subespacio utilizando solo las muestras etiquetadas que pertenecen a dicha variedad. De esta forma, se obtiene un conjunto de modelos supervisados specializados en cada subespacio, lo que puede mejorar el rendimiento del modelo global.

O el de \citet{demiriz1999semi} donde se utiliza un algoritmo de \textit{clustering} semi-supervisado que combina tanto la información de los datos no etiquetados como la información de las muestras etiquetadas para realizar la partición del espacio.
...
Este tipo de métodos es especialmente útil en el contexto de las \glspl{DNN}, donde la inicialización de los parámetros puede tener un gran impacto en el rendimiento del modelo supervisado, pues los algoritmos de entrenamiento pueden converger lentamente o presentar problemas de optimización. La idea general consiste en entrenar los parámetros de las capas inferiores de la \gls{DNN} utilizando los datos no etiquetados, y luego fine-tunear el modelo con los datos etiquetados \cite{hinton2006fast,vincent2008extracting}.

Para ilustrar este proceso desde un punto de vista matemático, el enfoque más representativo es el uso de \glspl{AE} apilados. Un \gls{AE} es una red neuronal diseñada para aprender la función identidad, es decir, para reconstruir su propia entrada, pero forzando al modelo a aprender una representación latente útil \cite{van2020survey}.

Formalmente, dado un conjunto de datos no etiquetados $D_u = \{x_1, \dots, x_m\}$ donde $x_i \in \mathbb{R}^d$, un \gls{AE} estándar se compone de dos funciones \cite{LI2023110176}:

\begin{enumerate}
    \item \textbf{El codificador (\textit{Encoder}) $h$}: Mapea el vector de entrada $x$ a una representación latente oculta $z \in \mathbb{R}^{d'}$, típicamente de menor dimensión ($d' < d$). Se define como:
    \begin{equation}
    ...
    \item \textbf{El decodificador (\textit{Decoder}) $g$}: Intenta reconstruir la entrada original a partir de la representación latente. Se define como:
    \begin{equation}
