...
Ahora, si los datos no son separables (esto es, no existe un hiperplano que separe perfectamente las clases), se introduce el \textit{soft margin} mediante variables de holgura $\xi_i\ge 0$ y un parámetro $C>0$ que controla el compromiso entre maximizar el margen y minimizar errores, tal y como se muestra a continuación.
\begin{equation}
\begin{aligned}
...
En el contexto de aprendizaje supervisado, el entrenamiento de una \gls{NN} consiste en ajustar los parámetros de los perceptrones, representados por una matriz $W$ que contiene tanto los pesos $\mathbf{w}$ como los sesgos $b$, para minimizar una función de pérdida \cite{van2020survey}.
