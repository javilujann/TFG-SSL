...
Además, se añade un término de regularización adicional, utilizando la matriz de pesos $W$ del grafo, que penaliza la falta de suavidad de $f$ respecto a la estructura del grafo:
\begin{equation}
    \|f\|_I^2 = \frac{1}{2} \sum_{i,j} w_{ij} \bigl(f(\mathbf{x}_i) - f(\mathbf{x}_j)\bigr)^2,
\end{equation}

este se puede interpretar como una medida de la variación de $f$ a lo largo de las aristas del grafo, ponderada por los pesos de similitud. De este modo, la función de pérdida total a minimizar es
\begin{equation}
