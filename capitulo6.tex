...
Esta familia de métodos se distingue de la anterior, porque no trata de modelar el proceso generativo y la densidad conjunta $p(\mathbf{x},y)$, sino que se centra directamente en aprender un modelo discriminativo $p(y\mid\mathbf{x})$ o una función de decisión $f:\mathcal{X}\to\mathcal{Y}$ \cite{ng2001discriminative}. En este caso, la información de los datos no etiquetados se incorporan con el objetivo de mejorar la generalización, normalmente a través de suposiciones adicionales sobre la estructura de los datos (por ejemplo, separación en baja densidad o la suavidad) \cite{van2020survey}.
