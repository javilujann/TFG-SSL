...
En la práctica, estos métodos pueden ser costosos computacionalmente, ya que suelen requerir operar con matrices de tamaño $n\times n$ (donde $n=l+u$). En particular, en algunos casos el coste puede alcanzar $O(n^3)$, o bien $O(n^2)$ \cite{van2020survey}. Además, el rendimiento es muy sensible a cómo se construye el grafo y a la elección y escala de los pesos $w_{ij}$; si el grafo no refleja bien la geometría de los datos, la información no etiquetada puede propagarse de forma perjudicial \cite{zhu2009introduction}.